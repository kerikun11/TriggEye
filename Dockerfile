...
COPY /data/patch_experts /data/patch_experts
...
ENV LD_LIBRARY_PATH=/usr/lib/aarch64-linux-gnu:/usr/local/cuda/lib:/usr/local/lib:$LD_LIBRARY_PATH
RUN mkdir -p opencv \
    ...
    && curl -fsSL https://github.com/opencv/opencv_contrib/archive/${OPENCV_VERSION}.tar.gz \
    ...
    -D CUDA_cufft_LIBRARY=/usr/lib/aarch64-linux-gnu/libcufft.so \
